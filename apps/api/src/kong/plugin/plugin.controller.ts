...
export class PluginController extends CommonController {
  /**
   * 构造函数
  ...
  async enum(
    ...
    const result = await this.objectService.plugin(instanceId);
    console.debug('enum', result);
  ...
  async list(
    ...
    res.locals.result = await this.objectService.list(instanceId, 'plugins');
  }
  ...
  async get(
    ...
    res.locals.result = await this.objectService.get(instanceId, 'plugins', id);
  }
