...
@Controller('kong/ssl')
export class SslController extends CommonController {
  /**
   * 构造函数
  ...
  async list(
    ...
    const result = await this.objectService.list(instanceId, 'certificates');
    res.locals.result = result.map((item: any) => ({
      ...item,
      key: undefined,
    }));
  }
